...
class HyperparameterXGBoost:
    ...
    def train(self, X_train, y_train):
        ...
        grid = GridSearchCV(XGBRegressor(), self.param_grid, cv=10, scoring='neg_mean_squared_error', n_jobs=-1)
...
class HyperparameterXGBoost_PSO:
    def __init__(self):
        self.scaler_X = MinMaxScaler()
        self.scaler_y = MinMaxScaler()
    ...
    def _objective_function(self, particles):
        losses = []
        for p in particles:
            # Extraemos y convertimos hiperparámetros
            n_estimators = int(np.clip(p[0], 50, 300))
            max_depth = int(np.clip(p[1], 2, 10))
            learning_rate = float(np.clip(p[2], 0.01, 0.3))
    ...
    def train(self, X_train, y_train, iters=50, swarm_size=20):
        ...
        # Definimos los límites de búsqueda: [n_estimators, max_depth, learning_rate]
        bounds = (np.array([50, 2, 0.01]), np.array([300, 10, 0.3]))
        ...
        self.best_params = {
            ...
            'learning_rate': float(best_position[2])
        }

        self.best_model = XGBRegressor(**self.best_params, verbosity=0, random_state=42)
